...
describe('HashSet', () => {
  ...
  test('insert()', () => {
    ...
    expect(() => set.insert([4, 13, 9], true)).toThrow(TypeError)
    expect(set.insert([16, 77, 12], false)).toBe(false)
    expect(set.insert({ a: 10 })).toBe(false)
    expect(() => set.insert(Map, true)).toThrow(TypeError)
    expect(() => set.insert(false, true)).toThrow(TypeError)
